...
impl Signer {
    ...
    pub fn new(secret: Vec<u8>, network: Network, tls: TlsConfig) -> Result<Signer, anyhow::Error> {
        ...
        //This handler is just used to get a static init msg from the signer
    ...
    async fn process_request(&self, req: HsmRequest) -> Result<HsmResponse, Error> {
        ...
        //The raw request is what's generated by core-lightning for delivery to hsmd
        ...
        //RANDY_TODO: Figure out use of approver. The approver and handler are
        //generated in each call so the approver isn't keeping state. The
        //invoice is also added directly to the node on the handler so that's
        //granting permission. Does the approver validate?
    ...
    //RANDY_COMMENTED
    //Returns the bip32 key from the vls-generated response to the init req.
    pub fn bip32_ext_key(&self) -> Vec<u8> {
        //Use Vls's messages
        use vls_protocol::{msgs, msgs::Message};

        //Set initmsg to the init response from vls
        let initmsg = msgs::from_vec(self.init.clone()).expect("unparseable init message");

        //match the initmsg based on the version of the reply and return the bip32 field
        //of the parsed init reply version
        match initmsg {
